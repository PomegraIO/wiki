...
Reads scripts/_seo4_proposals.json ; writes scripts/_seo4_plan.json .
...
def main() -> int:
    existing = set((SD / "_seo4_existing.txt").read_text(encoding="utf-8").split())
    data = json.loads((SD / "_seo4_proposals.json").read_text(encoding="utf-8"))
    ...
    for it in kept:
        ...
        it["rel"] = rel
    (SD / "_seo4_plan.json").write_text(json.dumps(kept, indent=1), encoding="utf-8")
