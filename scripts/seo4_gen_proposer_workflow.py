...
OUT = SD / "_seo4_proposer_workflow.js"
...
def main() -> int:
    briefs = json.loads((SD / "_seo4_briefs.json").read_text(encoding="utf-8"))
    jobs = []
    for b in briefs:
    ...
    js.append("  name: 'seo4-seo-proposers',")
    ...
    js.append("const CONC = 16;")
    js.append("const results = [];")
    js.append("for (let i = 0; i < JOBS.length; i += CONC) {")
    js.append("  const chunk = JOBS.slice(i, i + CONC);")
    js.append("  const part = await parallel(chunk.map(j => () =>")
    ...
    js.append("  results.push(...part);")
    js.append("  log(`proposed ${results.filter(Boolean).length}/${JOBS.length} buckets`);")
    js.append("}")
