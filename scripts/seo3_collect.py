...
Usage: python scripts/seo3_collect.py <workflow_dir>
Writes scripts/_seo3_proposals.json = { "buckets": [ {cat,sub,dir,proposals} ] }.
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: seo3_collect.py <workflow_dir>", file=sys.stderr)
        return 2
    ...
    for p in by_agent.values():
        ...
        total += len(props)
    (SD / "_seo3_proposals.json").write_text(
        json.dumps({"buckets": buckets, "totalProposals": total}, indent=1), encoding="utf-8")
    print(f"collected {len(buckets)} buckets, {total} raw proposals -> _seo3_proposals.json")
    return 0
