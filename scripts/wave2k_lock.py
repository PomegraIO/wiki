...
Reads scripts/_wave2k_proposals.json ; writes scripts/_wave2k_plan.json .
...
def main() -> int:
    existing = set((SD / "_wave2k_existing.txt").read_text(encoding="utf-8").split())
    data = json.loads((SD / "_wave2k_proposals.json").read_text(encoding="utf-8"))
    ...
    if len(kept) > TARGET:
        ...
        while excess > 0:
            for lst in order:
                live = [x for x in lst if id(x) not in drop]
                if len(live) <= 3:
                    continue
                drop.add(id(live[-1])); excess -= 1
                if excess == 0:
                    break
        kept = [x for x in kept if id(x) not in drop]
        print(f"trimmed to {len(kept)}")
    ...
    for it in kept:
        ...
        it["rel"] = rel
    (SD / "_wave2k_plan.json").write_text(json.dumps(kept, indent=1), encoding="utf-8")
