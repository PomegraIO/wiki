...
Usage: python scripts/seo3_make_writers.py [--batch 4] [--per-wave 130]
"""
...
def main() -> int:
    ...
    ap.add_argument("--plan", default="_seo3_plan.json", help="input plan/missing file")
    args = ap.parse_args()

    plan = json.loads((SD / args.plan).read_text(encoding="utf-8"))
    allow = [s for s in (SD / "_seo3_core_allowlist.txt").read_text(encoding="utf-8").split() if s]
    ...
    for wi, wave in enumerate(waves):
        ...
        js.append(f"  name: 'seo3-writers-w{wi}',")
