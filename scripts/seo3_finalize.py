...
Writes scripts/_seo3_missing.json for a targeted retry wave."""
...
def main() -> int:
    plan = json.loads((SD / "_seo3_plan.json").read_text(encoding="utf-8"))
    ...
    for it in plan:
        ...
        if kw and kw not in txt.lower():
            nokw += 1
    (SD / "_seo3_missing.json").write_text(json.dumps(missing, indent=1), encoding="utf-8")
