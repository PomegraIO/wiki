...
Outputs (scripts/):
  _wave2k_existing.txt        every slug on disk now (~17k after wave-3k) — global dedup set
  _wave2k_core_allowlist.txt  top-N most-linked concept slugs — writer cross-link vocabulary
  _wave2k_briefs.json         per (category,subcat) brief: intent-weighted quota + existing slugs
...
CORE_N = 360
TARGET_TOTAL = 2000
OVERASK = 2650  # sum of quotas before dedup/trim
...
def main() -> int:
    ...
    (SD / "_wave2k_briefs.json").write_text(json.dumps(briefs, indent=1), encoding="utf-8")
