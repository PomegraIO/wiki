---
title: "Expansionary Monetary Policy"
description: "Expansionary monetary policy is a central bank's effort to lower interest rates and increase the money supply to boost borrowing, spending, and economic growth."
keywords:
  - expansionary policy
  - monetary easing
  - low interest rates
  - recession
  - stimulus
image: "/svg/monetary.svg"
---

*An **expansionary monetary policy** — also called **monetary easing** — is a central bank's effort to lower interest rates, increase the money supply, and make credit cheaper and more available in order to spur borrowing, spending, investment, and economic growth. It is the standard policy response to a [recession](/recession/) or when unemployment is unacceptably high.*

<div class="wiki-hatnote">

This entry covers the general posture. For the specific tools a central bank uses to execute it, see [open-market operations](/open-market-operations/), [quantitative easing](/quantitative-easing/), and [forward guidance](/forward-guidance/).

</div>

<aside class="wiki-infobox">

<div class="wiki-infobox-title">Expansionary Monetary Policy — key facts</div>

<img src="/svg/monetary.svg" alt="Interest rate gauge pointing downward" />

<div class="wiki-infobox-caption">Expansionary policy lowers interest rates to stimulate growth and employment.</div>

|   |   |
|---|---|
| **Also called** | Monetary easing, loose monetary policy |
| **Objective** | Boost growth, reduce unemployment |
| **Typical conditions** | Recession, weak demand, rising unemployment |
| **Main tools** | Lower interest rates, asset purchases, forward guidance |
| **Effect on borrowing** | Makes loans cheaper and more available |
| **Transmission lag** | Six months to two years |
| **Main risk** | Excess inflation if continued too long |

</aside>

## When and why central banks ease

A central bank turns to expansionary policy when the economy is weak. The symptoms are familiar: unemployment is rising, business investment is falling, consumer spending is stalling, and the [inflation](/inflation/) rate is below target or falling. A [recession](/recession/) is either underway or feared. In such conditions, the central bank's goal is to make borrowing irresistibly cheap so that households will refinance mortgages, businesses will start new projects, and growth will recover.

The logic is straightforward: lower the cost of money, and people and firms will borrow more. More borrowing funds more spending. More spending means more sales, more hiring, and eventually more [inflation](/inflation/). The central bank trades a small increase in inflation for a large decrease in unemployment.

## The primary tool: cutting interest rates

The most visible move in expansionary policy is cutting the central bank's target [interest rate](/interest-rate/). In the United States, this is the [federal funds rate](/federal-funds-rate-target/). When the central bank cuts that rate, commercial banks' cost of overnight borrowing falls, and they in turn cut the rates they offer to customers: mortgages drop, credit-card rates fall, auto loans become cheaper. Refinancing and new borrowing accelerate.

The central bank typically cuts in a series of steps — cutting a quarter-point at a time, meeting by meeting, over a period of months — rather than in one dramatic move. This gradualism is partly signal: a series of small cuts says the central bank sees weakness but is not panicked. The pace also gives markets and the economy time to absorb each move.

## When rates hit zero: reaching for larger tools

If a recession is severe enough, the central bank can cut its target rate all the way to zero or even slightly negative, but it cannot go much further. Depositors and institutions will not accept large negative rates; they will demand cash instead. This **zero lower bound** is the hard floor. When rates reach zero and the economy is still weak, a central bank must reach for more-exotic tools.

**Quantitative easing** is the most famous. The central bank begins buying large quantities of [bonds](/bond/) — government securities, mortgage-backed securities, corporate [bonds](/bond/) — injecting enormous sums of cash into the financial system. These purchases drive down longer-term interest rates, pushing money toward riskier assets and toward lending to businesses and households.

**Forward guidance** — public statements about future rate policy — becomes more important when current rates are already at zero. If the central bank credibly promises to keep rates low for years, longer-term interest rates fall immediately, even before any new purchases happen.

## How the expansion filters through

When a central bank eases, the effects ripple outward in stages:

1. **Immediate.** Interest rates for mortgages, car loans, and credit cards fall. Refinancing becomes attractive; new borrowing becomes easier to justify.
2. **Weeks to months.** Households refinance; businesses dust off plans for new factories or equipment. Asset prices (stocks, real estate) often rise as investors hunt for returns in a low-rate world.
3. **Months to a year.** New borrowing drives new spending. Businesses hire; unemployment falls.
4. **A year onward.** Rising demand pushes up prices and [inflation](/inflation/) begins to accelerate.

The lag between the central bank's action and the real-world effect—typically six months to two years—is one reason central banks have to act on forecasts, not on what has already happened. A central bank tightening when it looks in the rear-view mirror will be too late.

## The risks: overshooting into inflation

The great danger of expansionary policy is overstaying. Cut rates too much or keep them low too long, and the economy can overheat. Demand outpaces supply, unemployment falls to unsustainably low levels, and firms raise prices faster and faster to cover rising labor costs. Before long, [inflation](/inflation/) spirals upward, eroding purchasing power and forcing the central bank into a painful tightening cycle. The transition from [contractionary monetary policy](/contractionary-monetary-policy/) back to normal then becomes a test of the central bank's credibility and pain tolerance.

## See also

<div class="wiki-seealso">

### Closely related

- [Monetary policy](/monetary-policy/) — the broader set of central bank tools
- [Contractionary monetary policy](/contractionary-monetary-policy/) — the opposite stance
- [Open-market operations](/open-market-operations/) — buying and selling securities
- [Quantitative easing](/quantitative-easing/) — large-scale asset purchases
- [Forward guidance](/forward-guidance/) — signaling future intentions
- [Federal funds rate target](/federal-funds-rate-target/) — the primary policy rate

### Wider context

- [Central bank](/central-bank/) — the institution that makes policy
- [Interest rate](/interest-rate/) — the price of money
- [Recession](/recession/) — the condition that triggers easing
- [Inflation](/inflation/) — the risk of overshooting
- [Bond](/bond/) — the assets central banks often buy

</div>
