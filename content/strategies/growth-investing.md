---
title: "Growth investing"
description: "Growth investing is a strategy of buying stocks with above-average earnings growth prospects, betting that the market will eventually reward the company's expanding profits with a higher valuation."
keywords:
  - growth investing
  - earnings growth
  - expansion strategy
  - growth stocks
  - capital appreciation
  - high-growth companies
image: "/svg/strategies.svg"
---

*Growth investing is a strategy centered on buying companies expected to grow their [earnings](/earnings-per-share/) significantly faster than the overall market or the economy, betting that expanding profits will eventually drive [capital gains](/stock/) regardless of the starting valuation.*

<div class="wiki-hatnote">

For the blend of growth and value, see [GARP](/garp/). For value-oriented strategies, see [value investing](/value-investing/). For the systematic factor version, see [momentum-factor](/momentum-factor/) or [profitability-factor](/profitability-factor/).

</div>

<aside class="wiki-infobox">

<div class="wiki-infobox-title">Growth investing — key facts</div>

<img src="/svg/strategies.svg" alt="A chart showing accelerating earnings and share price appreciation" />

<div class="wiki-infobox-caption">Growth investors follow accelerating earnings, riding expanding valuations upward.</div>

|   |   |
|---|---|
| **Core idea** | Buy earnings growth, not current cheapness |
| **Focus** | Revenue and profit expansion, market share gains |
| **Typical P/E** | Often well above market average, sometimes 30–50x |
| **Holding period** | Medium to long-term, often 5+ years |
| **Risk** | Valuation reset if growth slows, high volatility |
| **Key sectors** | Technology, healthcare, consumer discretionary |
| **Historical winners** | Amazon, Microsoft, Nvidia, Meta in growth phases |

</aside>

## The growth thesis

A growth investor believes that the market underweights future earnings potential. A company may look expensive by current [price-to-earnings ratio](/price-to-earnings-ratio/) standards, but if its profits are doubling every two to three years, a $100 stock growing 30% per year will justify $200, then $260, then $338 over five years — regardless of whether the multiple itself expands.

This is fundamentally different from [value investing](/value-investing/). A value investor buys cheapness; a growth investor buys *momentum in earnings*. The starting valuation is not the enemy — rapid earnings growth is the tailwind.

## What growth investors hunt for

- **Double-digit earnings growth.** Sustained profit expansion at 15%, 20%, 30% per year. The growth rate is often more important than the starting multiple.
- **Market share gains.** A company stealing customers from competitors and expanding its total addressable market (TAM).
- **Pricing power.** The ability to raise prices without losing customers, indicating a defensible competitive advantage.
- **Reinvestment opportunity.** The company has so many attractive uses for its capital that it can grow rapidly *and* reinvest heavily, reducing near-term dividends but expanding long-term value.
- **Secular tailwinds.** Growth trends (aging populations, cloud adoption, electrification, automation) that will compound the company's natural growth.

## Why growth stocks trade at a premium

A growth stock's high [price-to-earnings ratio](/price-to-earnings-ratio/) is not irrational. If you believe Company A will grow earnings 20% per year and Company B 3%, paying 40x for A and 12x for B may be perfectly reasonable. The multiple reflects the growth rate.

However, this creates vulnerability: if growth slows unexpectedly, the multiple can compress rapidly, creating a double hit — lower earnings *and* a lower multiple on those earnings. This is why growth stocks are often volatile, especially in sectors like technology.

## Growth versus [value](/value-investing/) — the long debate

Over decades, value stocks have outperformed growth on average, but this is not a law of nature. In certain eras — the 1980s, the 1990s, and 2010–2021 — growth has dominated. Growth stocks also exhibit lower volatility than their valuation multiples might suggest when the underlying growth is durable.

The deepest insight is that the distinction is temporal: today's growth stock becomes tomorrow's value stock. A company with 30% growth becomes a company with 10% growth, and at some point, a mature profitable business. The shift from growth to value is where mispricing opportunities often hide.

## See also

<div class="wiki-seealso">

### Closely related

- [GARP](/garp/) — growth at a reasonable price, the hybrid approach
- [Value investing](/value-investing/) — the alternative orientation
- [Momentum investing](/momentum-investing/) — riding trends in price and earnings
- [Quality-factor](/quality-factor/) — systematic screening for durable growers
- [Fundamental investing](/fundamental-investing/) — the analytical method

### Wider context

- [Earnings per share](/earnings-per-share/) — the growth metric
- [Price-to-earnings ratio](/price-to-earnings-ratio/) — the valuation ratio
- [Bull market](/bull-market/) — when growth tends to outperform
- [Asset allocation](/asset-allocation/) — sizing growth stocks in a portfolio

</div>
