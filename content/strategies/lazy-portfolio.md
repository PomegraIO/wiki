---
title: "Lazy portfolio"
description: "A lazy portfolio is an ultra-simple investment approach using one or two index funds on autopilot, designed for maximum convenience and minimum effort or decision-making."
keywords:
  - lazy portfolio
  - simple portfolio
  - one-fund portfolio
  - hands-off investing
  - set and forget
  - passive investing
image: "https://picsum.photos/seed/lazy-portfolio/900/600"
---

*A lazy portfolio is the simplest possible investment approach: holding one or two [index funds](/index-fund/) on complete autopilot, requiring no decisions, no rebalancing, and minimal attention. The investor contributes regularly and ignores market noise, letting compound growth work over decades.*

<div class="wiki-hatnote">

For slightly more sophistication, see [three-fund portfolio](/three-fund-portfolio/). For complete asset-allocation approaches, see [all-weather portfolio](/all-weather-portfolio/). For asset-allocation context, see [asset allocation](/asset-allocation/).

</div>

<aside class="wiki-infobox">

<div class="wiki-infobox-title">Lazy portfolio — key facts</div>

<img src="https://picsum.photos/seed/lazy-portfolio/900/600" alt="A single total-market index fund or two-fund portfolio held for decades" />

<div class="wiki-infobox-caption">Lazy investors hold one or two funds, rebalance never, and let compounding work.</div>

|   |   |
|---|---|
| **Components** | 1–2 index funds (total market stock, or stock + bonds) |
| **Time commitment** | Minimal; no rebalancing needed |
| **Costs** | Ultra-low; 0.03–0.10% expense ratios |
| **Suitability** | Hands-off investors, long-term holders |
| **Expected return** | Market returns; 9–10% stocks, 4–5% stock/bond blend |
| **Volatility** | Market volatility; 15–20% annually |
| **Complexity** | Extremely simple; one decision at setup |

</aside>

## The one-fund lazy portfolio

The absolute simplest lazy portfolio: hold a single total-market [index fund](/index-fund/) such as:

- **VTSAX** (Vanguard Total Stock Market Index Fund)
- **FSKAX** (Fidelity Total Market Index Fund)
- **VTI or VOO** ([ETF](/etf/) versions)

Or an all-in-one target-date fund:

- **VFIAX 2060** (Vanguard Target Retirement 2060)
- **FXAIX** (Fidelity target-date fund)

The investor:

1. Opens an account.
2. Sets up automatic monthly contributions.
3. Does nothing else for 30 years.
4. Retires with compound growth having done the work.

## The two-fund lazy portfolio

For risk management, a two-fund lazy portfolio splits between stocks and bonds:

- **80% total stock market index fund** (growth)
- **20% total bond market index fund** (stability)

Or automatically via a target-date fund (which internally manages the stock/bond split, gradually becoming more conservative).

## Advantages

- **Extreme simplicity.** One or two funds, no decisions.
- **Zero maintenance.** No rebalancing, no monitoring, no adjustment.
- **Ultra-low costs.** Index funds cost 0.03–0.10%; no advisory fees.
- **Behavioral advantage.** By removing all decisions, lazy investing prevents the worst investor behavior: panic selling, chasing performance, market timing.
- **Compounding.**  [Dividend reinvestment](/dividend-investing/) and automatic contributions leverage [compound interest](/compound-interest/).

## Disadvantages

- **No customization.** You get market returns, no tilt toward value, dividend, or specific sectors.
- **Volatility exposure.** A 100% stock lazy portfolio experiences 40–50% drawdowns in [bear markets](/bear-market/), potentially triggering panic.
- **Opportunity cost.** In bull markets, doing nothing costs you potential outperformance (though you avoid underperformance in downturns).
- **No income focus.** A lazy stock-heavy portfolio generates little [dividend](/dividend/) income for retirees.

## When lazy works best

- **For young investors.** Time horizon of 30+ years makes volatility irrelevant.
- **For those who cannot pick stocks.** Honest self-assessment: most investors cannot beat the market. Lazy beats picking.
- **For those lacking time/interest.** Busy professionals who value time over chasing returns.

## See also

<div class="wiki-seealso">

### Closely related

- [Three-fund portfolio](/three-fund-portfolio/) — slightly more sophisticated
- [Permanent portfolio](/permanent-portfolio/) — more complex all-weather
- [Index-fund](/index-fund/) — the core holding
- [Asset allocation](/asset-allocation/) — allocation context
- [Dollar-cost averaging](/dollar-cost-averaging/) — the contribution method

### Wider context

- [Stock](/stock/) — equity component
- [Bond](/bond/) — optional fixed-income component
- [Compound interest](/compound-interest/) — the growth engine
- [Bull market](/bull-market/) · [Bear market](/bear-market/) — market cycles

</div>
