---
title: "Merger arbitrage"
description: "Merger arbitrage is a strategy of buying stocks of companies being acquired at a discount to the acquisition price, betting on deal completion and capturing the spread as the deal closes."
keywords:
  - merger arbitrage
  - deal arbitrage
  - risk arbitrage
  - M&A trading
  - deal spread
  - acquisition premium
image: "https://picsum.photos/seed/merger-arbitrage/900/600"
---

*Merger arbitrage is a strategy of purchasing [stocks](/stock/) of companies announced for acquisition at a discount to the deal price, betting that the deal will close and capturing the spread between the current trading price and the acquisition price. The arbitrageur assumes risk that the deal fails or is delayed.*

<div class="wiki-hatnote">

For capital-structure arbitrage, see [capital-structure arbitrage](/capital-structure-arbitrage/). For broader arbitrage, see [statistical arbitrage](/statistical-arbitrage/) and [pairs trading](/pairs-trading/).

</div>

<aside class="wiki-infobox">

<div class="wiki-infobox-title">Merger arbitrage — key facts</div>

<img src="https://picsum.photos/seed/merger-arbitrage/900/600" alt="A target company trading below deal price pending regulatory approval" />

<div class="wiki-infobox-caption">Merger arbitrageurs pocket the spread between deal announcement and closing.</div>

|   |   |
|---|---|
| **Core idea** | Target trades below deal price; capture spread at closing |
| **Typical spread** | 1–5% depending on deal risk |
| **Holding period** | 3–12 months (until deal closes) |
| **Risk** | Deal fails, delayed, or renegotiated lower |
| **Return profile** | Consistent but modest (3–10% annually) |
| **Suitable for** | Deal-focused hedge funds, patient investors |
| **Disadvantage** | Concentrated deals reduce diversification |

</aside>

## How merger arbitrage works

**The setup:** Company ABC announces it will acquire Company XYZ for $100 per share. XYZ stock immediately rallies from $85 to $98 — near but not equal to the deal price. The $2 gap represents deal risk (the market is uncertain the deal will close).

**The trade:**

1. **Buy the target.** Purchase XYZ at $98.
2. **Wait for closing.** The deal requires regulatory approval, and that process takes 3–9 months.
3. **Capture the spread.** When the deal closes, XYZ is acquired and automatically converts to $100 cash per share.
4. **Profit:** $100 – $98 = $2 per share, or 2% return over the holding period.

For a 6-month holding period, a 2% return annualizes to ~4%.

## Deal risk

The merger arbitrageur assumes risk that:

- **Regulatory blocks the deal.** Antitrust concerns may cause regulators (FTC, DOJ, foreign antitrust authorities) to block the deal.
- **Financing fails.** If the acquirer is using debt or equity financing, market conditions can prevent financing completion.
- **Conditions are renegotiated.** The acquirer may demand a lower price (reputational loss for the arbitrageur).
- **Counterparty emerges.** Another bidder may trigger a bidding war (good!) or kill the deal (bad).
- **Delay.** A deal expected to close in 6 months may take 12+, increasing the waiting cost.

When a deal is blocked, the target stock crashes (back to $85 or lower), causing significant losses for arbitrageurs.

## Historical returns

Merger arbitrage has historically returned 5–10% annually with relatively low volatility. However, returns have compressed as more capital chases the same deals.

## See also

<div class="wiki-seealso">

### Closely related

- [Risk arbitrage](/merger-arbitrage/) — alternative term
- [Capital-structure arbitrage](/capital-structure-arbitrage/) — related arbitrage strategy
- [Pairs trading](/pairs-trading/) — relative-value trading
- [Statistical arbitrage](/statistical-arbitrage/) — broader arb strategies
- [Initial public offering](/initial-public-offering/) — inverse of M&A

### Wider context

- [Stock](/stock/) — the underlying instruments
- [Acquisition](/merger-arbitrage/) — the event
- [Deal risk](/merger-arbitrage/) — the core risk
- [Hedge fund](/hedge-fund/) — typical merger-arb venue

</div>
