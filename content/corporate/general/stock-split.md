---
title: "Stock Split"
description: "A stock split is a corporate action in which a company divides each outstanding share into multiple shares, increasing the total share count while keeping the total market capitalization unchanged. A 2-for-1 split means each old share becomes two new shares."
keywords:
  - stock split
  - share split
  - corporate action
...
|   |   |
|---|---|
| **What it is** | Division of each outstanding share into multiple shares |
| **Typical ratios** | 2-for-1, 3-for-1, or 4-for-1 |
| **Effect on value** | No effect; total value unchanged |
| **Effect on ownership** | No change; each shareholder's % ownership unchanged |
...
1. **Board approval.** The board proposes the split.
2. **Shareholder vote.** Shareholders vote to approve (may require majority or supermajority).
3. **Exchange notification.** The exchange is notified, and a new split-adjusted ticker is assigned.
4. **Record date.** A record date is set; shareholders on that date receive the split shares.
5. **Ex-date.** Shares trading ex-dividend (after the ex-date) trade at the new split-adjusted price.
6. **Payment.** The company distributes the new shares to shareholders.
