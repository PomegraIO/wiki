---
title: "Managed Float"
description: "A managed float (also called dirty float) is an exchange-rate system in which the currency floats freely most of the time but the central bank intervenes occasionally to smooth volatility or defend a preferred level."
keywords:
  - managed float
  - dirty float
  - guided float
...
|   |   |
|---|---|
| **What it is** | Float with occasional central-bank intervention |
| **Frequency** | Intermittent; not every day |
| **Purpose** | Smooth volatility; defend implicit level; lean against wind |
| **Tools** | Direct intervention; forward guidance; policy signaling |
| **Symmetry** | Can buy or sell currency depending on circumstances |
| **Typical target** | Trend level, not exact peg; tolerance band often implicit |
...
2. **The level drifts too far from trend** — even if the central bank doesn't announce a target, it has an implicit preference. If the currency appreciates beyond what fundamentals suggest, the central bank might sell its own currency to push it lower.

3. **A crisis threatens** — during a panic, the central bank may intervene heavily to prevent a freefall.
