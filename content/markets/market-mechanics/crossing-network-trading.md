...
The most significant regulatory burden is [Trade Reporting](/wiki/trade-reporting/). Every cross executed in a registered ATS must be reported to FINRA's Trade Reporting Facilities (TRF) or the exchanges within seconds of execution. These reports are time-stamped and feed into the consolidated tape that supplies prices to [market data vendors](/wiki/market-data-vendor/). This means crossing trades, though executed away from lit venues, contribute to the historical price record that informs future reference prices and technical analysis. The SEC views this transparency as a check on system abuse: if a crossing network systematically filled orders at prices worse than the best bid and ask it had access to, the pattern would be evident in the trade record.
