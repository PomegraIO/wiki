...
|   |   |
|---|---|
| **Frequency** | Rare; a few times per year across all US-listed stocks |
| **Trigger** | Trading halt followed by restart; order imbalance |
| **Duration** | Variable; typically a few minutes to an hour |
| **Participant scope** | All investors with outstanding orders or new orders |
| **Price impact** | Often large; reflects pent-up demand |
| **Regularity** | Unpredictable |
| **Announcement** | Disseminated immediately |
...
1. Trading is halted for the stock (or the entire market, in a system-wide halt).
2. The exchange announces that an auction will occur at a specific time (e.g., "Stock XYZ will reopen with an auction at 1:00 PM Eastern Time").
3. Investors submit or cancel orders ahead of the auction.
4. At the designated time, the exchange matches orders using the same mechanism as the opening auction: finding the clearing price.
5. Orders are executed at the clearing price; continuous trading resumes.
