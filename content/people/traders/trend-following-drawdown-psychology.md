---
title: "Trend Following Drawdown: Why Systematic Traders Endure Long Losing Periods"
description: "Trend-following drawdown explained: why systematic trend-following strategies suffer prolonged losing periods, how to identify normal drawdowns vs strategy failure, and historical examples."
keywords:
  - trend following drawdown how long
  - drawdown duration trend following
  - systematic trading losing periods
  - trend following strategy failure
  - drawdown recovery equity curve
...
<div class="wiki-infobox-title">Trend-Following Drawdown — metrics and psychology</div>

<img src="/svg/people.svg" alt="An abstract editorial mark for traders and systematic strategies." />

<div class="wiki-infobox-caption">Drawdowns are the price paid for capturing large trends; distinguishing normal from pathological is a key skill in systematic investing.</div>

|   |   |
|---|---|
| **Typical drawdown magnitude** | 20–50% in a single losing period |
| **Typical drawdown duration** | 6 months to 3+ years |
| **Frequency** | Multiple drawdowns per decade |
| **Cause** | Choppy markets, range-bound consolidation, [mean reversion](/momentum-investing/) |
| **Recovery time** | Often 2–5x the drawdown duration |
| **Trigger for strategy failure** | No profitable trend for 3+ years; [Sharpe ratio](/sharpe-ratio/) collapses |
| **Historical example** | Winton Capital drawdown 2007–2009: 28% loss, recovered by 2010 |
| **Investor risk** | [Liquidation](/liquidation/) during drawdown if panic redemptions overwhelm fund |
...
- **2010–2014**: After the 2008–2009 financial crisis, central banks (the Federal Reserve, European Central Bank, Bank of Japan) maintained ultra-low interest rates and conducted quantitative easing, dampening [volatility](/volatility-smile/) and creating choppy, range-bound trading. Trend-following strategies suffered significant drawdowns.
...
- **Financialization of commodities**: In 2007–2008, as institutional money poured into commodity index funds, [crude oil](/crude-oil/), [copper](/copper/), and agricultural commodities moved in lockstep with stock markets (correlation near 0.8). This co-movement broke natural hedges; a portfolio diversified across 50 commodities moved like a single asset, exacerbating drawdowns.
...
- **Drawdown severity relative to volatility**: A strategy with 20% annualized [volatility](/volatility-smile/) suffering a 25% drawdown is more normal than a strategy with 10% volatility suffering the same. The [Calmar ratio](/return-on-assets/) (return/max drawdown) captures this trade-off.
...
A [sharpe ratio](/sharpe-ratio/) (return per unit of risk) above 0.5 historically indicates a strategy that is operating within normal parameters despite drawdowns.
...
- **Characteristics**: Gradual erosion of returns over 3+ years; [Sharpe ratio](/sharpe-ratio/) collapses below 0.3; recovery to prior peak takes 5+ years or never occurs.
...
- **Example**: A mean-reversion fund that flourished in the 1980s–1990s might see its edge completely disappear if market noise and [algorithmic trading](/algorithmic-trading/) increase, making mean reversion less predictable.
- **Action**: Evaluate whether the strategy can adapt or if redemption is warranted.

The key diagnostic:

| Metric | Normal | Pathological |
|--------|--------|---|
| **Sharpe ratio during drawdown** | Stays above 0.3 | Falls below 0.2 |
| **Trend-by-year return** | Negative 1–2 years, then positive | Negative 3+ years in a row |
| **Correlation to [volatility](/volatility-smile/)** | Positive (profits in volatile markets) | Positive but muted (even in crises, returns are flat) |
| **Manager commentary** | Acknowledges specific headwinds; has a thesis for recovery | Vague, defensive, or no clear plan |
...
- **Redemption pressure**: When a trend-following fund drawdowns 30%, investors who had expected 12% annual returns panic. Redemptions spike; the fund must sell [liquid](/liquidity-risk/) positions, further damaging returns for remaining investors.
...
- **Forced adaptation**: Forced to reduce [position sizing](/position-sizing/) or add hedges to limit drawdown, which further reduces the strategy's return potential.
...
- **Adequate [capitalization](/capital-adequacy/)**: A fund that is undercapitalized may be forced to liquidate or close during a drawdown.
- **Transparent communication**: Managers who explain the structural causes of drawdowns to investors reduce panic redemptions.
- **Systematic risk management**: Strict position-sizing and [stop losses](/derivatives-hedging/) prevent a single drawdown from being catastrophic.
...
March 2020 was violent but brief; trend-followers captured profits in the initial panic. However, central bank stimulus created a subsequent period of "Goldilocks" returns—low [volatility](/volatility-smile/), moderate trends, no extremes. This choppy, "just right" market hurt momentum strategies. Drawdowns in 2021 were common, with recovery in 2022–2023 as [inflation](/inflation/) and rate-hike volatility returned.
...
- [Commodity trading advisor pioneer](/commodity-trading-advisor-pioneer/) — How CTAs manage and disclose drawdowns to investors
- [Floor trader vs screen trader](/floor-trader-vs-screen-trader/) — How traders in different eras psychologically cope with losing periods
- [Joe Ross trading educator](/joe-ross-trading-educator/) — Pattern-recognition rules as a method to reduce [whipsaw losses](/bid-ask-spread/) during choppy markets
- [Volatility smile](/volatility-smile/) — Understanding implied volatility and option pricing during market stress
- [Momentum investing](/momentum-investing/) — The flip side of trend-following; mean reversion profits in choppy, range-bound markets

### Wider context

- [Value at risk](/value-at-risk/) — Quantifying portfolio drawdown exposure
- [Sharpe ratio](/sharpe-ratio/) — Assessing strategy quality independent of recent performance
- [Diversification](/diversification/) — How broad [portfolio construction](/asset-allocation/) reduces single-period drawdowns
- [Behavioral economics](/loss-aversion/) — Why investors panic during drawdowns despite historical recovery
- [Futures contract](/futures-contract/) — The leveraged instruments amplifying trend-follower drawdowns and gains
