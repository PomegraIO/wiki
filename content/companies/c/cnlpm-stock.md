---
title: "Connecticut Light & Power Co (CNLPM)"
description: "Connecticut Light & Power (CL&P), operating as Eversource Energy, is a regulated electricity utility serving more than 1.2 million customers across Connecticut, operating under rate regulation by the Public Utilities Regulatory Authority."
keywords:
  - regulated utility
  - electricity distribution
  - Connecticut
  - Eversource
  - public utility
  - energy distribution
handwritten: true
---

[Connecticut Light & Power](/cnthp-stock/) Company, trading under the ticker CNLPM as [preferred stock](/preferred-stock/), is a regulated electricity distributor serving Connecticut. The company operates as a subsidiary of [Eversource Energy](/es-stock/) and rebranded under that corporate name in 2015, though the underlying operating utility retains its legal identity. It is one of two major investor-owned electric distribution companies in Connecticut, serving approximately 1.2 million customers across roughly 149 towns and 4,400 square miles of the state.

<aside class="wiki-infobox">
<table>
<tr><th>Ticker</th><td>CNLPM (preferred stock)</td></tr>
<tr><th>Parent company</th><td>Eversource Energy</td></tr>
<tr><th>Service territory</th><td>Connecticut (149 towns, 4,400 sq. mi.)</td></tr>
<tr><th>Customers served</th><td>1.2+ million electricity</td></tr>
<tr><th>Regulatory agency</th><td>Connecticut PURA</td></tr>
<tr><th>Primary business</th><td>Electricity distribution</td></tr>
<tr><th>SEC CIK</th><td>0000023426</td></tr>
</table>
</aside>

## A regulated monopoly with energy choice

As a regulated utility, Connecticut Light & Power operates under a franchise to serve all customers in its territory without competition for the distribution network itself—the poles, wires, and transformers that carry electricity to homes and businesses. The Connecticut Public Utilities Regulatory Authority (PURA) sets the rates the company can charge, ensuring cost recovery with a mandated [return on equity](/return-on-equity/), and approves capital investments. This regulatory model provides revenue predictability: once PURA approves rates, the company collects them for several years regardless of cost [inflation](/inflation/) or volume changes.

However, the state's electricity market operates under partial deregulation. Since 2000, residential and business customers in Connecticut have had "Energy Choice"—the ability to buy their electricity from suppliers other than the utility. What the utility always provides is the physical infrastructure: the wires, the meters, and the grid operations. This two-tier system means Connecticut Light & Power keeps steady revenue from distribution and grid services while competing suppliers sell the commodity power. For the utility, this reduces the risk of margin compression from commodity price volatility but also caps upside from rising prices.

## Capital needs and modernisation

The electrical grid in Connecticut is decades old. Distribution utilities across the Northeast, including Connecticut Light & Power, are investing heavily in replacing aging infrastructure, hardening the grid against storm damage, and integrating distributed solar and battery storage. These capital projects are costly and ongoing. The regulatory framework allows utilities to recover these costs through rates, but there is a lag—the utility must spend capital, wait for PURA approval, then collect through higher rates—which creates timing risk and requires external financing.

Connecticut Light & Power is part of a much larger parent company, Eversource Energy, which also operates utilities in Massachusetts and New Hampshire. This corporate structure allows the parent to allocate capital and manage debt across three states and multiple regulatory jurisdictions, but it also means the preferred stock of Connecticut Light & Power is subordinated to the consolidated credit and [dividend](/dividend/) policy of the parent. The parent company decides capital allocation and distributions.

## The competitive and regulatory landscape

Two investor-owned distributors serve Connecticut: Eversource (Connecticut Light & Power) and The United Illuminating Company. Both operate under PURA regulation and both must navigate the increasing complexity of grid modernisation, renewable energy integration, and potential electrification of heating and transportation. Regulators in Connecticut have adopted performance-based regulation frameworks that tie cost recovery and returns more directly to service quality and decarbonization metrics, adding operational scrutiny beyond simple cost-plus rate-setting.

The renewable energy transition creates both opportunity and risk. Utilities benefit from the capital they must invest to upgrade distribution, and they earn returns on that capital under regulation. But the long-term electricity demand picture is uncertain—electric vehicles and heat pumps could increase overall demand, or aggressive efficiency and on-site solar could flatten or reduce it. Customer-owned rooftop solar is already reducing electricity sales per customer, a phenomenon called "volumetric decline" that regulators address through changes to the rate structure.

## How to research Connecticut Light & Power

Begin with the annual 10-K filing of the parent company, Eversource Energy (SEC CIK 0000065984), which consolidates the results of all three states' operations and provides detail on capital expenditures, regulatory dockets, and cost-structure trends. Watch the investor presentations and regulatory filings in front of PURA—these often lay out rate cases and major capital projects. Track the dividend policy of Eversource Energy and note any changes to payouts or guidance, as these signal the parent's view of future cash generation. Monitor news on regulatory decisions affecting grid modernisation, renewable energy integration standards, and any changes to the rate-setting formula. The distribution utility's competitive position depends on service reliability (track outage frequency and duration), cost efficiency (operating expense per customer), and regulatory relationships—all visible in PURA dockets and investor communications rather than in Wall Street consensus estimates.
