---
title: "Leverage Shares 2X Long CBRS Daily ETF (CBRG)"
description: "A daily-reset leveraged ETF tracking Cboe BRR (Bitcoin Volatility Risk Premium Index) with 2x long exposure — designed for tactical traders, not buy-and-hold investors."
keywords:
  - leveraged ETF
  - Bitcoin volatility
  - daily reset
...
| | |
|---|---|
| **What it is** | 2x leveraged daily reset ETF |
| **Underlying** | Cboe BRR (Bitcoin Volatility Risk Premium Index) |
| **Leverage** | 2x daily, reset each day at close |
| **Expense ratio** | Higher than unleveraged; covers borrowing costs |
| **Best for** | Tactical traders, not long-term holders |
| **Primary risk** | Volatility decay; loses value in choppy, sideways markets |
| **Structure** | Uses derivatives and leveraged lending, not margin |
...
## How 2x daily reset leverage works and why it decays

CBRG does not borrow money to buy leverage. Instead, it uses derivatives — futures, swaps, and options — to construct a position that moves 2x the daily return of the BRR index. If the index gains 1% in a day, CBRG aims to gain 2%. If the index loses 1%, CBRG aims to lose 2%.
...
CBRG works in two scenarios. First, a sustained directional move in the BRR index — a rally or a decline that compounds over days without reversing. Leverage amplifies the gain (or loss). Second, a very smooth transition, where the index climbs steadily with minimal backpedaling. The daily reset is irrelevant when there is little to reset.

CBRG breaks in several ways. A whipsaw market, where the index bounces up and down, erodes the fund through decay, delivering negative returns even if the index is flat. A mean-reverting market, where prices spike and then retrace, also generates decay. And a sustained down market with high volatility decay is brutal: the fund loses twice as fast and rebalances by selling at the worst times.

Beyond volatility decay, CBRG has a secondary risk: the index itself can mean-revert violently. The volatility risk premium is powerful in calm markets and catastrophic in panics. A sudden spike in Bitcoin's realized volatility, or a sudden drop in implied volatility (which should not happen but can), flips the index sharply negative. A 2x leveraged fund in that scenario compounds the loss.
...
The prospectus and fact sheet lay out the leverage mechanics, the reset schedule, and the costs. The most important research is historical: examine what happened to the fund's price versus the underlying index in calm periods and in volatile periods. The pattern usually shows underperformance in sideways markets and outperformance (or less underperformance) in directional moves. Watching the fund during real volatility events — a crypto crash, a panic in Bitcoin options — reveals whether the leverage amplifies your edge or just your pain. And crucially, compute the cost of entry and exit: what is the bid-ask spread? How much volume is there? Getting caught in a leveraged position with no liquidity to exit is a road to ruin.
