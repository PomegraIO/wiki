---
title: "GraniteShares Autocallable MARA ETF (MRA)"
description: "A structured ETF from GraniteShares linked to Marathon Digital Holdings, using an autocallable note mechanism to offer conditional coupons and partial downside cushion on bitcoin mining exposure."
keywords:
  - structured product
  - autocallable
  - bitcoin mining
...
An autocallable note can be terminated early if certain market conditions are met. In MRA's case, the note is linked to Marathon's stock price and accrues a [coupon payment](/coupon-payment/) each period as long as the stock stays above a trigger barrier—typically around 65-70 per cent of the note's original price. If Marathon's stock rallies and crosses a call barrier—often set at 150 per cent of the original price—the note is automatically called, the coupon is paid, and the investor receives their principal back.
...
GraniteShares has issued multiple [tranches](/tranche/) of MRA over time, each with slightly different terms. The call price, [coupon rate](/coupon-rate/), and trigger barrier shift with market conditions and [implied volatility](/implied-volatility/). A tranche issued when mining sentiment is optimistic might carry a higher call price (more generous to buyers) and lower coupon. One issued when sentiment is cautious might carry a lower call price (more restrictive) and higher coupon. Each iteration represents GraniteShares' assessment of the implied value of the structure given current market dynamics.

The fund carries an [expense ratio](/expense-ratio/) listed on the prospectus, but that is only a small part of the cost. The true value extraction happens in the difference between the coupon GraniteShares pays and the cost of hedging the structure; that spread is the issuer's profit, embedded in the note's pricing and not fully transparent to the buyer.
...
Read the current term sheet carefully: the call price, coupon rate, trigger level, and any other conditions define what you actually own. Compare that coupon to the yield available from owning Marathon stock directly ([dividend](/dividend/) plus expected capital appreciation) or from alternative mining exposures. Check the [secondary market](/secondary-market/) [bid-ask spread](/bid-ask-spread/) to understand the cost of exit; structured products often trade wider than ordinary ETFs. Be explicit about your thesis: are you expecting mining to boom (own the stock), to crash (own [bonds](/bond/) or cash), or to churn sideways (where the coupon might make sense)? MRA makes sense only in that third scenario, and even then only if you genuinely believe the coupon compensates for the upside cap you are accepting.
