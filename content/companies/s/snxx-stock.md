...
<table>
<tr><th>Ticker</th><td>SNXX (NASDAQ)</td></tr>
<tr><th>Issuer</th><td>Tradr / ETF Sponsor</th></tr>
<tr><th>Underlying</th><td>SanDisk Corporation (SNDK) stock</td></tr>
<tr><th>Leverage ratio</th><td>2X daily</td></tr>
<tr><th>Reset frequency</th><td>Daily (rebalanced at market close)</td></tr>
<tr><th>Mechanics</th><td>Options, swaps, and margin derivatives</td></tr>
<tr><th>Best suited to</th><td>Intraday and short-term traders only</td></tr>
<tr><th>Expense ratio</th><td>High (cost of leverage and rebalancing)</td></tr>
</table>
</aside>

## The mechanics of daily leverage

SNXX does not simply borrow money to hold twice as many SNDK shares (that would be traditional margin leverage, and it would work differently). Instead, it uses a combination of derivatives — options, [total return swaps](/total-return-swap/), and synthetic contracts — to structure a portfolio that mimics double the daily return of SNDK.
...
The most important fact about SNXX is that **it loses value if you hold it through volatility, even if SNDK stays flat**. This is called volatility decay, and it is baked into the structure of any [leveraged ETF](/leveraged-etf/).
...
Day one: SNDK rises 10%; SNXX aims to rise 20%, from $100 to $120.
Day two: SNDK falls 10%; SNXX aims to fall 20%, from $120 to $96.
...
The [expense ratio](/expense-ratio/) for SNXX is quite high — typically 1% or more annually — because the fund must constantly rebalance its derivatives, pay counterparties, and hedge its exposure. Those costs are in addition to the volatility decay that eats away at long-term returns.

The other risk is [counterparty risk](/counterparty-risk/): derivatives are contracts with financial institutions. If the counterparty (a bank or derivatives [broker](/broker/)) fails or becomes unable to pay, you could lose money. This risk is small for a major ETF issuer, but it is non-zero and higher than simply owning stock.

A final risk is that the fund might shut down. Leveraged single-stock ETFs have modest assets compared to broad [index funds](/index-fund/), and a fund with low assets might be liquidated by its sponsor. If that happens, you are forced to exit at the [liquidation](/liquidation/) price, which may not be favourable.
