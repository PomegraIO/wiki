---
title: "Simplify US Equity PLUS Bitcoin Strategy ETF (SPBC)"
description: "An options-based ETF that aims to provide US equity exposure plus returns from selling call options on Bitcoin, seeking to enhance yield through structured call-writing."
keywords:
  - ETF
  - options strategy
  - bitcoin exposure
...
The Simplify US Equity PLUS [Bitcoin](/bitcoin/) Strategy ETF (ticker SPBC) tackles a problem many equity investors face: the gap between the income an equity portfolio generates and the yields available in [bonds](/bond/). It does this by holding a core US equity portfolio and simultaneously selling [call options](/call-option/) on Bitcoin — a strategy that collects [option premiums](/option-premium/) to boost total return in flat or declining cryptocurrency markets, but surrenders gains if Bitcoin rallies sharply.
...
This is a covered-call strategy applied to a non-correlated asset class. [Covered calls](/covered-call/) are a standard equity income tactic — an investor owns stock and sells call options on it to generate premium income. Here, the "covered" part is inverted: the fund owns equity but sells calls on Bitcoin. If Bitcoin stays flat or falls, the fund keeps both the equity returns and the full option premium, enhancing yield. If Bitcoin rallies sharply past the call [strike price](/strike-price/), the fund's upside is capped; the call will be exercised, and the fund forgoes the remaining gains.

## Structure and costs

SPBC is a standard ETF, trading daily on an exchange. The fund carries an [expense ratio](/expense-ratio/) that covers the cost of rebalancing, research, and the implementation of the options strategy. The real cost, though, is the opportunity cost: when Bitcoin rallies, investors in SPBC will lag far behind Bitcoin itself and even behind a simple long Bitcoin position.
