---
title: "Global X SuperDividend REIT ETF (SRET)"
description: "An exchange-traded fund tracking real estate investment trusts screened for above-average dividend yields; provides exposure to REITs with high cash distributions to shareholders."
keywords:
  - REIT
  - dividend yield
  - real estate
...
<table>
<tr><th>Ticker</th><td>SRET (NASDAQ)</td></tr>
<tr><th>Issuer</th><td>Global X ETFs</td></tr>
<tr><th>Fund type</th><td>REIT income ETF</td></tr>
<tr><th>Asset class</th><td>Real estate investment trusts</td></tr>
<tr><th>Selection criteria</th><td>High dividend yield</td></tr>
<tr><th>Typical holding count</th><td>30–50 REITs</td></tr>
<tr><th>Trade venue</th><td>NASDAQ</td></tr>
</table>
</aside>
...
## How SRET selects its holdings

The fund applies a straightforward selection rule: it screens the broad REIT universe (which includes hundreds of publicly traded REITs across many property types and geographies) and selects those with above-average dividend yields. The exact methodology varies with market conditions, but the core principle is that SRET owns the REITs paying the highest cash distributions relative to their share prices.

This yield-focused approach has important implications. A REIT's dividend yield rises when its stock price falls. So a REIT that was previously yielding three percent can jump to five percent if investors bid down its stock price by forty percent. SRET will own more of the fund's assets in that REIT once its yield climbs. This is a value-tilting characteristic: the fund naturally accumulates positions in beaten-down REITs that are yielding heavily, often for good reasons (weak property markets, management questions, specific property concerns). It will underweight or exit REITs whose yields have compressed as their stock prices have risen, which are often the momentum winners.
...
SRET shareholders receive distributions that reflect the high dividend yields of the underlying REITs. Because those yields are paid out regularly (typically quarterly), the fund generates substantial income for shareholders. This is attractive for retirees or other income-seeking investors, but it comes with a tax cost: REIT dividends are typically taxed as ordinary income (not capital gains) in non-retirement accounts.

The fund's own yield (the dividend it pays, expressed as a percentage of its share price) fluctuates. If SRET's share price falls, the yield rises; if the share price rises, the yield compresses. If the underlying REITs cut their dividends (which can happen in downturns), SRET's distributions fall.

The total return from SRET includes both the income paid and the change in the share price. In rising real estate markets, REIT share prices rise, adding to total return. In falling markets, share prices fall, offsetting or exceeding the income received. A high-dividend fund in a declining market can see its share price fall faster than the dividend cushions the loss.
...
Investors considering SRET should begin with Global X's factsheet and prospectus, which lay out the selection methodology, the current expense ratio, and the risk disclosures. The holdings list shows what REITs the fund actually owns and the yields it is capturing.

Comparing SRET's distribution history, share-price performance, and total return over time against both a broad REIT index ETF and other income-focused REIT funds will reveal whether the high-yield tilt has justified its approach. Examining the concentration in specific property types and the average yield being captured will help assess the trade-off between income and risk.

Finally, understanding what economic environment SRET is most sensitive to is essential: it tends to thrive when real estate values are appreciating and interest rates are stable or falling, and struggles when real estate faces headwinds or rates are rising sharply.
