---
title: "StockSnips AI-Powered Sentiment US All Cap ETF (NEWZ)"
description: "NEWZ is an actively managed ETF that uses artificial intelligence and sentiment analysis to select US companies of all market capitalizations. It aims to outperform the broad market through algorithmic stock picking."
keywords:
...
NEWZ is an **actively managed exchange-traded fund** sponsored by StockSnips that applies artificial intelligence and sentiment analysis to select stocks from across the US market — large cap, mid cap, and small cap companies without restriction. Rather than tracking a preset index, the fund's algorithm analyzes sentiment signals from news, social media, and other data sources to identify which companies the market may be mispricing. The manager rebalances regularly based on those signals, betting that systematic sentiment analysis can beat the returns of a passive index.

## The algorithm and how it chooses stocks
...
## How to research NEWZ

Anyone considering NEWZ should review the fund's prospectus and fact sheet for the exact methodology, the algorithm's rules, and the fee structure. Comparing NEWZ's returns over multiple years against a simple US broad-market ETF is essential: if NEWZ does not beat that benchmark net of fees, the active approach has failed. The fund publishes the algorithm's training data and historical backtest — readers should ask what portion of outperformance came from overfitting to the past versus genuine predictive signals.
