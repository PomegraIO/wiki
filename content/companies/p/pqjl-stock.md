---
title: "PGIM Nasdaq-100 Buffer 12 ETF - July (PQJL)"
description: "PGIM Nasdaq-100 Buffer 12 ETF - July is a structured ETF providing Nasdaq-100 exposure with a 12-percent downside buffer and an annual reset tied to July, balancing protection with upside participation."
keywords:
  - buffer ETF
  - Nasdaq-100
  - capital protection
...
PQJL holds a long position in the Nasdaq-100 (via a swap, a fund, or direct equity holdings), paired with a zero-cost collar constructed from options. A zero-cost collar means the manager buys puts (downside protection) and sells calls (upside cap) in quantities that offset the [option premiums](/option-premium/), so the net cost is zero rather than paid from the [expense ratio](/expense-ratio/).

The puts are struck to protect against losses exceeding 12 percent; the calls are struck at a level that caps the fund's upside close to the index's nominal gain (the exact cap depends on volatility and [interest rates](/interest-rate/) at the reset date). Each July, both the puts and calls expire and new ones are written. A July reset that occurs after the market has rallied benefits from lower volatility, typically enabling a tighter upside cap (less protection cost, more favorable for equity holders). A July reset after a decline benefits from higher volatility, making puts more expensive and caps tighter, less favorable for equity investors.
