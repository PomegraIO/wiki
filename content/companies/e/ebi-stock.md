---
title: "Longview Advantage ETF (EBI)"
description: "EBI is a low-volatility equity ETF that screens for stocks exhibiting both strong recent performance and lower sensitivity to market swings, combining momentum and defensive characteristics."
keywords:
  - low volatility
  - momentum
  - defensive strategy
  - quality equity
  - systematic selection
  - equity ETF
handwritten: true
---

The Longview Advantage ETF, trading under the ticker EBI, is an equity fund built on a simple idea: find stocks that have been doing well recently AND that are less jumpy than the average stock. It is not trying to time the market or pick winners by guessing. Instead, it runs the numbers every few months to identify companies that combine recent positive price momentum with lower day-to-day volatility. The fund then holds those stocks in equal-weight positions, which means you own the same dollar amount of each company rather than having the largest ones dominate.

<aside class="wiki-infobox">
<table>
<tr><th>Ticker</th><td>EBI (NASDAQ)</td></tr>
<tr><th>Issuer</th><td>Longview Asset Management</td></tr>
<tr><th>Structure</th><td>Equity ETF (systematic, rules-based)</td></tr>
<tr><th>Number of holdings</th><td>Approximately 50–100</td></tr>
<tr><th>Expense ratio</th><td>Modest to moderate</td></tr>
<tr><th>Weighting</th><td>Equal weight, rebalanced quarterly</td></tr>
<tr><th>Rebalancing</th><td>Quarterly</td></tr>
</table>
</aside>

## The strategy in plain language

Here is what EBI does. At the start of each quarter, it looks at all stocks in its investable universe and scores them on two things. First, it looks at how they have performed over the past few months — stocks that are up get a higher score. Second, it measures how volatile each stock has been. A volatile stock jumps around a lot on bad news; a stable one does not. EBI wants stocks with decent recent gains but lower jumps. Once the fund identifies the companies that hit both targets, it buys them all in equal-size chunks.

The equal-weight approach is crucial. A cap-weighted fund (which owns companies in proportion to their total value) naturally becomes dominated by the largest companies. EBI avoids that. Every stock gets the same slice. If you own EBI, you own the same dollar amount of a small-cap equal-opportunity business and a mid-cap industrial manufacturer. This structure means the fund is more exposed to smaller companies and is more volatile than a cap-weighted version of the same stock screen would be.

## Why momentum and volatility matter together

Momentum is a real pattern. Stocks that have been going up tend to keep going up for a while — not because of magic, but because rising earnings, management execution, or investor enthusiasm creates feedback that carries a stock higher. Many funds have been built on this insight alone. The twist EBI adds is to filter for stocks that are also less volatile. A volatile stock that went up recently may have done so in a wild, jerky fashion that suggests shakier fundamentals. A stock with solid recent gains but quieter day-to-day movement suggests steadier improvement. By combining the two signals, the fund aims to catch stocks in a genuine uptrend rather than ones that are just bouncing around.

The screening is mechanical and transparent — there is no forecasting or judgment about which company will do better. It is pure signal-chasing. That means EBI will work well when recent stock performance is a reliable guide to the future, and will lag when new information breaks hard against last quarter's winners.

## Composition and concentration risk

EBI typically holds between 50 and 100 stocks, depending on how many pass the dual screen at each rebalance. That is smaller than a broad market fund but large enough to reduce [idiosyncratic risk](/idiosyncratic-risk/). However, equal weighting a smaller group of stocks creates more concentration than a cap-weighted broad fund. A 1/50th position in each of 50 companies is far more concentrated than a 1/500th position in 500 companies, even if you have more holdings.

The fund's sectors will shift based on which industries are in momentum. In a technology boom, EBI might become overweight tech simply because tech stocks are going up and not yet volatile. In a value rotation, defensives may dominate. That dynamic is baked into the strategy — EBI does not cap sectors to a fixed allocation.

## Costs and trading

EBI's [expense ratio](/expense-ratio/) is kept modest by the fund's straightforward, rules-based process. There are no discretionary decisions, no analyst calls, no active trading. The main costs come from quarterly rebalancing (buying and selling shares to reset positions to equal weight) and the [bid-ask spread](/bid-ask-spread/) when you buy or sell ETF shares. For most investors, the spread is tight enough not to matter unless you are trading very large blocks.

[Dividends](/dividend/) from holdings pass through to shareholders, usually quarterly. Because ETFs are structurally efficient at minimizing distributions, you are unlikely to face big taxable capital-gains surprises in a taxable account. The equal-weight rebalancing does mean more turnover than a passive buy-and-hold, which can matter if you are in a high tax bracket.

## The real risks

Here is what can hurt EBI. If recent winners become tomorrow's losers — if the momentum signal breaks down — the fund will hold exactly the wrong stocks at the worst time. Momentum is a real pattern, but it is not permanent. In markets that reward unpopular, beaten-down stocks (a value rotation), EBI's recent winners may continue falling.

Volatility decay is not a risk here (that is only for leveraged and inverse funds). But equal weighting a smaller number of stocks means the fund is more volatile than a cap-weighted broad alternative. If you are nervous during downturns, that matters. A sharp market correction may hit EBI harder because it is overweight smaller, more volatile names.

There is also a [liquidity risk](/liquidity-risk/): some of the smaller stocks EBI may hold are not liquid. In a market panic when everyone wants to sell, EBI may face wider spreads or difficulty trading its positions smoothly.

## Who this is for and how to research

EBI fits investors who believe in momentum as a strategy, want lower volatility than the broad market, and can tolerate the extra concentration and choppiness of equal weighting. It is not for buy-and-hold investors who simply want the whole market at the lowest cost — that person should own a market [index fund](/index-fund/). EBI is also not ideal for someone who cannot handle seeing their portfolio drop 20 percent in a bad quarter, because equal-weight momentum portfolios can be volatile even with the volatility screen built in.

To research EBI, start with the fund's fact sheet, which will show you the exact selection criteria and the current holdings. Look at the recent performance during both up markets and downturns. Compare it to a simple momentum fund (without the volatility filter) and to a simple low-volatility fund (without the momentum filter) to see whether the combination is actually working. Check the prospectus for the exact rebalancing schedule and expense ratio. Read the fund company's white papers or research notes on why momentum and volatility together make sense — the logic should be clear and the evidence credible. Over time, track whether the fund delivers steadier returns than it would using momentum alone, or whether the volatility screen is just diluting the signal.
