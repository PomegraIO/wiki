---
title: "Innovator U.S. Equity Buffer ETF – March (BMAR)"
description: "An actively managed ETF that aims to cushion losses in U.S. stocks while participating partially in gains, using options strategies and monthly reset windows."
keywords:
  - buffer ETF
...
This is not a guarantee. The buffer is not insurance purchased from an insurer — it is an options trade designed to approximate that protection. Losses beyond the buffer level can still occur, particularly in flash-crash environments where markets move so fast that options markets dislocate or in scenarios where the fund cannot execute its hedges as planned.
...
## Who the March series is for

Buffer ETFs appeal to investors who want to own stocks but sleep better knowing their losses are bounded. A retiree in drawdown phase, for example, might prefer capped downside to unhedged equity risk. An investor who sold stocks at a peak and wants to re-enter the market gradually might use a buffer ETF to ease back in while controlling downside. Advisors sometimes use buffer ETFs as a tactical allocation within larger portfolios — swapping in and out based on market conditions.

The March series is the first in Innovator's lineup, hitting specific buffer and cap levels each month. Other series in the family target different buffers (April, May, and others each have their own target level) and different underlying indexes.
...
## Research pathways for buffer ETF investors

Anyone considering BMAR should read Innovator's prospectus and the most recent fact sheets to understand the current month's collar structure. It helps to run a mental backtest: imagine various market scenarios (a 5% drop, a 15% drop, a 20% rally) and sketch what the buffer and cap would mean for returns. Historical fact sheets show how the fund actually performed in past volatile months — that real data is more instructive than the marketing pitch.

The key insight is that buffer ETFs are not for buy-and-hold-forever investors; they are tactical tools for managing specific risks in specific time periods.
