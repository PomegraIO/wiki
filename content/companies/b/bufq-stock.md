---
title: "FT Vest Laddered Nasdaq Buffer ETF (BUFQ)"
description: "An ETF that seeks to replicate the upside of the Nasdaq-100 while capping downside losses to around 13% annually through structured options strategies."
keywords:
  - buffered ETF
  - defined outcome
  - Nasdaq-100
...
<table>
<tr><th>Fund name</th><td>FT Vest Laddered Nasdaq Buffer ETF</td></tr>
<tr><th>Ticker</th><td>BUFQ</td></tr>
<tr><th>Sponsor</th><td>Equinix Fund Trust (managed by First Trust Advisors)</td></tr>
<tr><th>Index tracked</th><td>Nasdaq-100</td></tr>
<tr><th>Strategy</th><td>Capped upside with downside buffer via options</td></tr>
<tr><th>Buffer level</th><td>~13% annual downside protection</td></tr>
<tr><th>Structure</th><td>Plain vanilla ETF (not leveraged, not inverse)</td></tr>
<tr><th>Expense ratio</th><td>Roughly 1% annually (reflects options costs)</td></tr>
</table>
</aside>

## How the buffer works

The heart of BUFQ is an options-based overlay. Instead of holding the 100 stocks that make up the Nasdaq-100 outright, the fund holds the index (or financial instruments that replicate it) and layers on a series of call and [put options](/put-option/). The puts — the right to sell at a floor price — are the mechanism that caps losses. The fund purchases out-of-the-money puts that trigger if the index falls beyond roughly 13%, protecting holders from losses beyond that threshold. To fund the cost of these puts without eroding returns too much, BUFQ sells (writes) [call options](/call-option/) — limits on how much gains it captures if the index moves more than a certain amount upward in that year.

This layered construction is called "laddered" because the fund uses multiple tiers of calls and puts, each resetting annually. Rather than a single fixed cap and floor, the ladder creates a range, and the construction is recalibrated each year. The precise cap on gains and floor on losses shift based on how volatility moves and where [interest rates](/interest-rate/) settle, but the intent stays consistent: meaningful participation in rallies, real loss mitigation in crashes.

## Structure and mechanics

BUFQ is not a leveraged fund, not an inverse or short product, and not an exchange-traded note (ETN). It is a conventional ETF, which means it holds securities and options directly and passes the legal structure through to investors. This matters for tax treatment and for safety: you own the underlying assets, not a promise from an issuer. The fund trades like any other ETF — on exchanges, with real-time pricing and [bid-ask spreads](/bid-ask-spread/) — so you can buy or sell intraday rather than settling at a once-daily [net asset value](/net-asset-value/).
