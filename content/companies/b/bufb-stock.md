---
title: "Innovator Laddered Allocation Buffer ETF (BUFB)"
description: "An ETF that holds stocks with built-in downside protection, resetting the protection monthly to limit losses if the market drops, in exchange for capping gains."
keywords:
  - downside protection
  - buffer strategy
  - stock market
...
<table>
<tr><th>Ticker</th><td>BUFB (NASDAQ)</td></tr>
<tr><th>Fund sponsor</th><td>Innovator ETFs</td></tr>
<tr><th>What it holds</th><td>Stocks with integrated put options (downside protection)</td></tr>
<tr><th>Protection level</th><td>Buffer against losses up to a certain threshold</td></tr>
<tr><th>Reset frequency</th><td>Monthly</td></tr>
<tr><th>Upside cap</th><td>Limited gains in strong months</td></tr>
<tr><th>Structure type</th><td>Equity with embedded options</td></tr>
</table>
</aside>

## How the buffer works

Think of it this way: the fund buys stocks, then buys insurance (in the form of [put options](/put-option/)) to protect against big drops. The put option says "if the stock falls below this price, I get paid the difference." That payment cushions your loss. In return, the fund gives up some of the gains if stocks rally hard — the fund accepts a cap on how much profit it can make each month.
...
The fund resets the protection and return cap monthly. This matters because options (the insurance contracts) have [expiration dates](/expiration-contracts/). Every month the fund lets the old contract expire, collects or pays whatever is owed, and buys a new set of options for the next month at current prices and market conditions. This keeps the protection fresh and calibrated to the current stock-market level.
...
BUFB trades on the [NASDAQ](/nasdaq/) with decent volume, so getting in and out is usually straightforward. The [expense ratio](/expense-ratio/) reflects active management and the ongoing cost of options trading. Over many years, those costs compound, so compare BUFB's total returns to a regular stock fund, not just its costs — the protection might or might not be worth what you pay.
