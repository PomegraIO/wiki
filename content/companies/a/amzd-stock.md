---
title: "Direxion Daily AMZN Bear 1X ETF (AMZD)"
description: "An inverse ETF that aims to track negative one times the daily performance of Amazon stock, rising when AMZN falls. Intended for tactical hedging or bearish bets over short periods."
keywords:
  - inverse ETF
  - Amazon stock
  - bearish hedge
...
The Direxion Daily AMZN Bear 1X ETF (AMZD) is an inverse exchange-traded fund that moves in the opposite direction of Amazon shares. When Amazon rises 1% in a trading day, AMZD aims to fall roughly 1%. When Amazon falls 1%, AMZD aims to rise roughly 1%. Unlike a leveraged fund, AMZD uses a 1X multiplier — it simply inverts the daily move — making it a hedge or a tactical bearish position rather than an amplified bet. Like other daily-reset inverse funds, it is designed for short-term tactical use, not for holding across extended periods.
...
AMZD holds derivatives, primarily swap contracts and possibly short stock positions, that produce negative exposure to Amazon. If Amazon is up, the derivatives lose money, and AMZD's value declines. The fund is rebalanced daily to maintain a target inverse ratio, ensuring that the next day's opening sees a reset to exactly -1X relative to Amazon's close.

## The daily reset problem for inverse funds

An inverse fund held over weeks or months encounters the same volatility decay problem as leveraged funds, but in reverse. Consider a simplified example: Amazon trades at 100, falls to 90 (down 10%), then rallies back to 100. A holder of AMZD experiences: inverse gain on the down move (good), then inverse loss on the up move (bad) from a higher base after the daily reset, resulting in a net loss even though Amazon ended at 100. In ranging markets where the stock oscillates, AMZD bleeds value from the daily rebalancing.

The longer AMZD is held, the worse this decay typically becomes. A trader using AMZD for a one-day bearish bet incurs minimal drag. A holder over months will almost certainly find that the inverse exposure has underperformed a direct short of the same magnitude due to this mechanical property.
