---
title: "Avantis International Small Cap Value ETF (AVDV)"
description: "AVDV is a value-focused ETF holding small-cap companies from developed markets outside the US, filtered by low valuations and quality metrics."
keywords:
  - small-cap value
...
A broad small-cap index holds the entire universe of small-cap stocks weighted by market capitalization. A value-tilted fund like AVDE weights the same universe slightly more toward cheap stocks but keeps most of the breadth. AVDV is more concentrated: it filters first for value, then weights the remaining stocks accordingly. This means AVDV will have substantially fewer holdings than a broad small-cap fund and will own far fewer expensive growth companies. In absolute numbers, AVDV might hold 200–300 stocks compared to 500–1000 in a broad index. The result is higher tracking error relative to the broader market and a sharper value tilt, which can drive outperformance during years when value factors lead but underperformance when growth leads. A value-tilted fund's performance is partly a bet on whether the value factor itself will outperform over time.
...
AVDV is for investors with long time horizons (10+ years) who want to express a conviction that small-cap value stocks will outperform the broader market and who have the risk tolerance to endure periods of sharp underperformance. It works best as a satellite holding within a diversified portfolio, not as a core equity position. Investors should be comfortable with higher volatility and the possibility of trailing the broader market for years at a time.

To evaluate AVDV, start with its prospectus and fact sheet from American Century Investments, which detail the specific value metrics used to filter and weight holdings. Examine the top holdings and sector breakdown to understand the concentration and how different sectors are represented. Compare the fund's valuation metrics (average price-to-book, price-to-earnings) to both a broader small-cap index and peer value funds to confirm the tilt is genuine. Review the fund's trailing returns and volatility relative to indexes like the MSCI World Small Cap Index to understand its performance in different market environments. Track the factor performance environment — periods when value factors outperform or underperform — to contextualize the fund's results.
