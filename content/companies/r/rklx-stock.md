---
title: "Defiance Daily Target 2X Long RKLB ETF (RKLX)"
description: "RKLX is a leveraged ETF that tracks Rocket Lab (RKLB) with 2x daily amplification, using derivatives and daily rebalancing to double daily gains and losses—a short-term trader's tool, not a long-term holding."
keywords:
  - leveraged ETF
  - 2x leverage
  - Rocket Lab
  - daily reset
  - volatility decay
  - short-term trading
handwritten: true
---

*The Defiance Daily Target 2X Long RKLB ETF (RKLX) is a leveraged exchange-traded fund that aims to deliver twice the daily return of [Rocket Lab](/rklb-stock/) stock (RKLB). Using derivatives and daily rebalancing, the fund amplifies both the gains and the losses of the underlying aerospace company's share price.*

## What the fund tracks and its structure

RKLX holds no actual Rocket Lab shares. Instead, it uses derivatives—primarily futures, swaps, and other financial instruments—to achieve a leveraged exposure to RKLB. If RKLB rises 1%, RKLX aims to rise 2%. If RKLB falls 1%, RKLX aims to fall 2%. The fund's sponsor, Defiance ETFs, dynamically adjusts the fund's derivative positions each trading day to maintain this 2x exposure ratio.

This daily reset is the fund's defining feature. At the market close each day, the fund recalibrates its leverage so that the next trading day begins fresh with a 2x target. The daily reset is necessary to maintain a consistent [leverage ratio](/leverage-ratio-forex/), but it has a crucial and often misunderstood consequence: in markets that oscillate, the fund can underperform the 2x target over periods longer than a single day. This slippage is called volatility decay, and it is intrinsic to [leveraged ETF](/leveraged-etf/) design.

## The mechanics of volatility decay and why it matters

Suppose RKLB rises 10% on Day 1 and falls 10% on Day 2. The stock ends flat. But RKLX, resetting daily, experiences a different path: it rises 20% on Day 1 (2x the 10% gain), then falls 20% on Day 2 (2x the 10% loss). Starting with 100, after Day 1 RKLX is at 120. After Day 2, it is at 96. The stock is flat, but RKLX has lost 4%.

This dynamic worsens in choppy, sideways markets where daily moves cancel each other out. Over weeks or months, even if the underlying stock is modestly positive, RKLX can lag substantially behind 2x of the stock's gain because volatility decay compounds. This is not a flaw; it is a direct consequence of daily rebalancing in a volatile asset.

## Investor profile and intended use

RKLX is designed for traders with a specific thesis and a short time horizon—days to a few weeks. If you believe RKLB will rally 10% in the next fortnight and want to make that move with less capital than buying the stock directly, RKLX offers that lever. The fund is liquid, tradeable during market hours, and transparent in its daily rebalancing.

RKLX is emphatically not suitable for long-term investors. A buy-and-hold investor in Rocket Lab should buy RKLB directly and avoid leverage altogether. The volatility decay of leverage works as a drag over months and years, even if the underlying stock appreciates. Worse, leverage amplifies downside with the same intensity as upside: a 50% decline in RKLB becomes a 100% wipeout in RKLX.

## Risk and leverage amplification

The fundamental risk is that leverage cuts both ways. If RKLB moves against you, your loss doubles. A 10% drop becomes 20%. A 25% decline becomes 50%. Position sizing is critical; investors in RKLX must hold far smaller positions than they would in RKLB itself, and they must maintain strict stop-losses. Holding a 2x leveraged position through a 20% downswing in the underlying stock is a recipe for severe capital loss.

A secondary risk emerges during market stress. If RKLB gaps down at the open (due to a company announcement or a broad market crash), the fund may face margin demands from its derivatives counterparties. In extreme scenarios, the fund itself may be forced to rebalance at unfavorable prices, crystallizing losses at precisely the moment an investor least wants to sell.

Additionally, trading liquidity for RKLX can dry up in volatile periods. The [bid-ask spread](/bid-ask-spread/)—the difference between the price at which you can buy and the price at which you can sell—widens when trading volume drops, meaning your entry and exit costs can be significant during a market move.

## Costs and trading characteristics

RKLX carries an [expense ratio](/expense-ratio/) higher than an unleveraged [equity ETF](/equity-etf/), reflecting the cost of maintaining the daily derivative positions and rebalancing. The exact ratio is published in the fund's prospectus and fact sheet. Because leverage-enabled daily rebalancing is an active process (the fund must adjust its positions every day), the cost is meaningful, though still modest in absolute terms.

The fund trades throughout the market day like any ETF, but there is an important caveat: the NAV diverges from the daily 2x target during the trading day. RKLX only resets its leverage at the close of trading, so during hours, intraday performance may lag or lead the 2x target depending on how RKLB is moving.

## How to research RKLX

Start by understanding Rocket Lab itself—read the company's quarterly earnings reports, track its launch cadence and customer wins, and monitor industry sentiment around small-satellite launch. Then read RKLX's prospectus, paying particular attention to the volatility-decay and daily-reset sections. The prospectus will also spell out the fund's expense ratio and any restrictions on redemptions or daily trading limits.

The Defiance website publishes RKLX's fact sheet and daily performance data. Compare RKLX's returns over the past month or quarter against 2x the returns of RKLB; the gap is volatility decay in action. Back-testing RKLX's theoretical performance through periods of RKLB volatility—big up moves followed by down moves, or prolonged sideways trading—will show concretely how leverage behaves in different market regimes. If your thesis is a quick directional move, that is the only horizon over which RKLX is appropriate.
