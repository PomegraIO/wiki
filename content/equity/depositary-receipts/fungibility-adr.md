...
---
*A **fungible ADR** is one that can be freely swapped, share for share, with the foreign ordinary shares it represents. Fungibility is the quiet plumbing that lets an [American depositary receipt](/adr/) trade in New York at a price that never strays far from the same company's stock in its home market — because the moment it does, someone can convert one into the other and pocket the difference.*

<aside class="wiki-infobox">

<div class="wiki-infobox-title">ADR Fungibility — key facts</div>

<img src="/svg/equity.svg" alt="An abstract editorial mark for cross-border equity ownership." />

<div class="wiki-infobox-caption">The convertibility that ties a New York receipt to a share trading an ocean away.</div>

|   |   |
|---|---|
| **What it is** | Two-way convertibility between an ADR and its underlying foreign shares |
| **Why it matters** | Keeps the ADR price tethered to the home-market price |
| **Enforced by** | Arbitrage traders and the depositary bank |
| **Core actions** | Issuance (shares → ADRs) and cancellation (ADRs → shares) |
| **Limited by** | Local capital controls, taxes, and registration rules |
| **Holder is called** | An ADR holder, economically a shareholder |

</aside>

## What "fungible" actually means here

To call an ADR fungible is to say it is interchangeable with the ordinary share underneath it. One ADR might represent one home-market share, or a fraction, or several — the ratio is set when the programme is created — but whatever the ratio, the receipt and the shares it stands for are two wrappers around the same economic claim. Hold the ADR and you hold the dividends, the votes, and the residual interest of the foreign [stock](/stock/); the receipt is merely the form in which a US investor takes delivery.
...
- [American Depositary Receipt](/adr/) — the instrument that fungibility makes convertible
- [Stock](/stock/) — the underlying ownership claim an ADR represents
- Shareholder — what an ADR holder economically is
- Arbitrage — the trade that enforces the price link
- [Bid-Ask Spread](/bid-ask-spread/) — part of the friction that leaves a residual gap
- [Broker](/broker/) — how a domestic investor buys an ADR

### Wider context

- [Stock Exchange](/stock-exchange/) — the home and host venues an ADR bridges
- [Dividend](/dividend/) — the income that passes through to the ADR holder
- [Market Capitalization](/market-capitalization/) — the company-level value an ADR scales into
